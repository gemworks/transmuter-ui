...
export default function TransferTransmuterOwnership({ isOpen, toggleModal, transmuterWrapper }: TransferTransmuterOwnershipProps) {
  ...
  useEffect(() => {
	...
	async function transferTransmuterOwnership_() {
		toast
			.promise(
				transferTransmuterOwnership(publicKey),
				{
					pending: `Transfering ownership to ${publicKey}`,
					success: `Successfully transfered ownership to ${publicKey}`,
							...
							<div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
								<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
									<div className="sm:flex sm:items-start">
										...
										<div className="my-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
											...
											<div className="">
												<p className="text-sm text-gray-500 my-2">Are you sure you want to transfer your Transmuter to someone else? This action cannot be undone.</p>
												<InputField
													inputFieldTitle={"New Transmuter Owner"}
													publicKey={publicKey}
								...
								<div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse ">
									<button
										...
										onClick={transferTransmuterOwnership_}
									>
										Transfer Ownership
									</button>
									<button
										...
										onClick={() => {toggleModal(); resetPublicKey();}}
										ref={cancelButtonRef}
									>
										Cancel
									</button>
