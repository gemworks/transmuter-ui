...
export function RadioButtons({ whitelistType, setWhitelistType }: RadioButtonProps) {
  ...
  return (
    <div>
      ...
      <p className="text-sm leading-5 text-gray-500">[Filter Type Placeholder]</p>
          ...
          {notificationMethods.map((notificationMethod, index) => (
            <div key={notificationMethod.id} className="flex items-center">
              <input
                ...
                onClick={(e: any) => { setWhitelistType(e.target.value) }}
...
export function InputField({ publicKey, handlePublicKeyChange, setPublicKey, resetPublicKey }: InputFieldProps) {
  ...
  // const [publicKey, handlePublicKeyChange, resetPublicKey] = useInputState("");
...
export default function SlideOver({ open, toggleState, bankPk, addToWhitelist, removeFromWhitelist, addRarities, transmuterWrapper }: SlideOverProps) {
  ...
  async function removeFromBankWhitelist_(mint: string) {
    const { tx } = await transmuterWrapper.removeFromBankWhitelist(
                  ...
                  <div>
                    ...
                    <div className="mt-6 relative flex-1 px-4 sm:px-6 overflow-hidden">
                      ...
                      <div className="space-y-6">
                        ...
                        <RadioButtons
                          whitelistType={whitelistType}
                          setWhitelistType={setWhitelistType}
                        />

                        <button
                          onClick={() => { addToWhiteList_() }}
                          disabled={publicKey.length < 15}

                          className="disabled:opacity-75 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-150 ease-in"
                        >
                          Add Token
                        </button>
                      </div>
                    ...
                    <div className="mt-6 relative flex-1 px-4 sm:px-6 overflow-y-scroll">
                      <div className="mt-20 flex-1 overflow-y-scroll">
                        <h3 className="font-medium text-gray-900 mb-2">Whitelisted Addresses</h3>

                        {whiteList.map((item, index) => <ul className="border-t border-b border-gray-200 divide-y divide-gray-200" key={index}> <Icon
                          publicKey={item.publicKey} whiteListType={item.whiteListType} removeFromWhiteList={() => removeFromBankWhitelist_(item.publicKey)} /></ul>)}
